...
- MCP surfaces live in `app/mcp/user_server.py`, `app/mcp/admin_server.py`, and multi-client tool modules such as [`app/mcp/chatgpt/visit_tools.py`](../app/mcp/chatgpt/visit_tools.py). The MCP servers use `AppsSDKFastMCP` (FastMCP 3.0.1) with Streamable HTTP transport and support any MCP-compatible client (ChatGPT, Claude, Cursor, VS Code, Gemini, etc.).
