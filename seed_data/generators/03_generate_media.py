#!/usr/bin/env python3
"""
Generate placeholder media files for seed data.

Scans all seed JSON files for "media/" references and creates
small placeholder images (JPEG/PNG) and minimal PDFs in seed/media/.
Uses Pillow for image generation. Files are tiny (~500 bytes each).
"""

from __future__ import annotations

import re
from pathlib import Path

SEED_DIR = Path(__file__).resolve().parent.parent / "seed"
HARDCODED_DIR = Path(__file__).resolve().parent.parent / "hardcoded"
MEDIA_DIR = SEED_DIR / "media"

# Color palette for generating visually distinct placeholder images
CATEGORY_COLORS: dict[str, tuple[int, int, int]] = {
    "avatars": (70, 130, 180),       # Steel blue
    "properties": (60, 179, 113),    # Medium sea green
    "tours": (218, 165, 32),         # Goldenrod
    "floor_plans": (147, 112, 219),  # Medium purple
    "documents": (205, 92, 92),      # Indian red
    "blogs": (255, 140, 0),          # Dark orange
}

IMAGE_SIZE = (64, 64)  # Small placeholder size


def _extract_media_refs() -> set[str]:
    """Scan all seed + hardcoded JSON files and extract unique media/ references."""
    refs: set[str] = set()
    pattern = re.compile(r'"(media/[^"]+)"')

    for json_dir in (SEED_DIR, HARDCODED_DIR):
        if not json_dir.exists():
            continue
        for json_file in json_dir.glob("*.json"):
            text = json_file.read_text(encoding="utf-8")
            for match in pattern.findall(text):
                refs.add(match)

    return refs


def _create_placeholder_image(path: Path, color: tuple[int, int, int]) -> None:
    """Create a small solid-color JPEG or PNG image using Pillow."""
    try:
        from PIL import Image, ImageDraw

        img = Image.new("RGB", IMAGE_SIZE, color=color)
        draw = ImageDraw.Draw(img)
        # Add a small label
        label = path.stem[:8]
        draw.text((4, IMAGE_SIZE[1] // 2 - 6), label, fill=(255, 255, 255))

        img.save(str(path), quality=50)
    except ImportError:
        # Fallback: write minimal valid JPEG bytes
        # Minimal 1x1 JPEG
        minimal_jpeg = bytes([
            ,
        ])
        path.write_bytes(minimal_jpeg)


def _create_placeholder_pdf(path: Path) -> None:
    """Create a minimal valid PDF file."""
    content = (
        "%PDF-1.0\n"
        "1 0 obj<</Type/Catalog/Pages 2 0 R>>endobj\n"
        "2 0 obj<</Type/Pages/Kids[3 0 R]/Count 1>>endobj\n"
        "3 0 obj<</Type/Page/MediaBox[0 0 612 792]/Parent 2 0 R>>endobj\n"
        "xref\n0 4\n"
        "0000000000 65535 f \n"
        "0000000009 00000 n \n"
        "0000000058 00000 n \n"
        "0000000115 00000 n \n"
        "trailer<</Size 4/Root 1 0 R>>\n"
        "startxref\n190\n%%EOF\n"
    )
    path.write_text(content, encoding="utf-8")


def _get_category(path_str: str) -> str:
    """Extract the media category from a path like 'media/properties/...'."""
    parts = path_str.split("/")
    if len(parts) >= 2:
        return parts[1]
    return "other"


def main() -> None:
    """Generate all placeholder media files."""
    refs = _extract_media_refs()
    if not refs:
        print("No media references found in seed JSON files.")
        return

    created = 0
    skipped = 0

    for ref in sorted(refs):
        target = SEED_DIR / ref
        if target.exists():
            skipped += 1
            continue

        target.parent.mkdir(parents=True, exist_ok=True)
        category = _get_category(ref)
        color = CATEGORY_COLORS.get(category, (128, 128, 128))

        ext = target.suffix.lower()
        if ext == ".pdf":
            _create_placeholder_pdf(target)
        else:
            _create_placeholder_image(target, color)

        created += 1

    print(f"Media generation complete: {created} created, {skipped} already existed ({len(refs)} total references)")


if __name__ == "__main__":
    main()
